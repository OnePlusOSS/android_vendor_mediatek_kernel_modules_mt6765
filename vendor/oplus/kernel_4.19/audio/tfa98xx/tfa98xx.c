...
static int oplus_audio_mic_mode_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = s_mic_mode;
	return 0;
}

static int oplus_audio_mic_mode_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int oplus_audio_headset_mic_mode_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = s_headset_mic_mode;
	return 0;
}

static int oplus_audio_headset_mic_mode_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int oplus_audio_soundtrigger_info_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = s_soundtrigger_enable;
	return 0;
}

static int oplus_audio_soundtrigger_info_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static const struct snd_kcontrol_new oplus_audio_hardware_resource_controls[] = {
	SOC_SINGLE_EXT("Audio_Mic_Mode", SND_SOC_NOPM, 0, 6, 0, oplus_audio_mic_mode_get, oplus_audio_mic_mode_set),
	SOC_SINGLE_EXT("Audio_Headset_Mic_Mode", SND_SOC_NOPM, 0, 6, 0, oplus_audio_headset_mic_mode_get, oplus_audio_headset_mic_mode_set),
	SOC_SINGLE_EXT("Audio_Soundtrigger_Info", SND_SOC_NOPM, 0, 1, 0, oplus_audio_soundtrigger_info_get, oplus_audio_soundtrigger_info_set),
};
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,19,0)
void oplus_create_hardware_resource_controls(struct snd_soc_codec *codec)
{
   struct snd_soc_component *cmpnt = &codec->component;
#else
void oplus_create_hardware_resource_controls(struct snd_soc_component *component)
{
   struct snd_soc_component *cmpnt = component;
#endif
	snd_soc_add_component_controls(cmpnt, oplus_audio_hardware_resource_controls, ARRAY_SIZE(oplus_audio_hardware_resource_controls));
}
#endif
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
    ...
    if (tfa98xx->tfa->dev_idx == 0) {
	#if LINUX_VERSION_CODE < KERNEL_VERSION(4,19,0)
        oplus_create_hardware_resource_controls(tfa98xx->codec);
	#else
        oplus_create_hardware_resource_controls(tfa98xx->component);
	#endif
	}
#endif
...
static int tfa98xx_parse_dual_spk(struct device *dev, struct tfa98xx *tfa98xx)
{
	int ret = 0;
	u32 dual_spk = 0;
	char *tfa_dual_spk = "oplus,tfa98xx-dual-spk";
			     ...
			     const struct i2c_device_id *id)
{
	...
	#ifndef OPLUS_BUG_COMPATIBILITY
	prEntry_temp = proc_create("oplus_tfa98xx_fw_update", 0644, prEntry_tfa9890,&proc_firmware_update);
	#endif /* OPLUS_BUG_COMPATIBILITY */
